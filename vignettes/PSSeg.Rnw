...
The preprocessing before segmentation consists in three steps:
\begin{enumerate}
\item $\log$-transform total copy numbers to stabilize their variance, this step improve segmentation results for all methods
\item smooth outliers using {\code{smooth.CNA}} \cite{olshen04circular} by way of the option \code{dropOutliers} in \code{PSSeg} function (the default values is \code{TRUE}.
\item allelic ratios are converted to (unimodal) decrease in heterozygosity ($d$) in \code{PSSeg} function.
\end{enumerate}
