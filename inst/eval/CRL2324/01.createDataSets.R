...
regA = data.frame(region = unique(dat$region), freq = c(0.90/4,0.90/4,0.90/4,0.90/4,0.10/3,0.10/3,0.10/3))
...
for (bb in 1:B) {
  ...
  if (!file.exists(pathname) || simForce) {
    simTag100 <- sprintf("ROC,n=%s,K=%s,regSize=%s,minL=%s,pct=100,ponderation", len, K, regSize, minL)
