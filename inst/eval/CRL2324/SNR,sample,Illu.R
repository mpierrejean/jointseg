...
for(methTag in methTags){
  ...
  pathnameBkp <-  sprintf("../SimulatedChr1Illumina/bkpData/CRL2324,BAF,ROC,n=2e+05,K=20,regSize=0,minL=100,pct=%s,ponderation",pp)
  snrbymeth=data.frame(bkp=NA,SNR=NA,status=NA, reg1 = NA, reg2=NA, len1=NA, len2=NA)
  for(bb in 1:50){
    dataBkp <- loadObject(sprintf("%s/CRL2324,BAF,ROC,n=2e+05,K=20,regSize=0,minL=100,pct=%s,ponderation,b=%s,%s.xdr",pathnameBkp,pp,bb,methTag))
    SNRs <-SNRResults[[bb]]
...
### Tableaux
matRegTot <- table( resReg[["reg1"]],resReg[["reg2"]])[c(2,3,5,6),c(2,3,5,6)]
...
matReg <- table( resRegMissedBymeth[[methTag]][["reg1"]],resRegMissedBymeth[[methTag]][["reg2"]])[c(2,3,5,6),c(2,3,5,6)]
...
rownames(res) <- gsub("log","",gsub("+DP","",gsub("|het","",gsub("\\(Kmax=200\\)","",methTags))))
xtable(res)
...
image(t(log(matSNRbkpMissed)),col = terrain.colors(100))
matplot(t(log(matSNRbkpMissed)),col = terrain.colors(100))
