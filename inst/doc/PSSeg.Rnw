...
\maketitle
\begin{abstract}
This vignette describes how to use the \code{PSSeg} function from the \code{jointSeg} package to partition bivariate DNA copy number signals into segments of constant parent-specific copy number.   The proposed method \cite{pierre-jean:jointSeg} starts by identifying a list of candidate change points through a fast (greedy) recursive binary segmentation (RBS).  This list is then pruned using dynamic programming \cite{bellman56dynamic}.  The resulting approach is able to quickly and accurately identify true changes in DNA copy numbers. 
...
The resulting copy-number profile is plotted in Figure \ref{fig}.
<<profile-fig, echo=TRUE, fig=TRUE>>=
plotSeg(datS, sim$jumps)
@ 

\section{PSSeg segmentation}
